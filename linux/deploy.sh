...
HEALTH_CHECK_FILE_DIR=/www/wwwroot/zipkin   # 脚本会在这个目录下生成nginx-status文件
APP_HOME=/www/wwwroot/zipkin  # 从package.tgz中解压出来的jar包放到这个目录下
JAR_NAME=zipkin.jar # jar包的名字
...
start_application() {
    ...
    java -jar ${APP_HOME}/${JAR_NAME} --server.port=8402 --zipkin.storage.type=mysql --zipkin.storage.mysql.db=febs_cloud_base --zipkin.storage.mysql.username=febs_cloud_base --zipkin.storage.mysql.password=Psi5GEwW4f2nEwMs --zipkin.storage.mysql.host=localhost --zipkin.storage.mysql.port=3306 --zipkin.collector.rabbitmq.addresses=localhost:5672 --zipkin.collector.rabbitmq.username=febs  --zipkin.collector.rabbitmq.password=123456  > ${APP_LOG} 2>&1 &
    echo $! > ${PID_FILE}
}
stop_application() {
    echo "stop jar"
